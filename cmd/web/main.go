...
func main() {
	...
	http.Handle("/rss", sentryMiddleware.HandleFunc(func(w http.ResponseWriter, r *http.Request) {
		feed := &feeds.Feed{
			...
			Description: "Latest blacklist events of USDT/USDC on Ethereum, Arbitrum, Base, Optimism, Avalanche, Polygon, ZkSync, Linea and Tron networks",
		}
	...
	http.Handle("/sitemap.xml", sentryMiddleware.HandleFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		urls := []urlEntry{
			...
			{Loc: baseURL + "/?page=1&chain=linea"},
